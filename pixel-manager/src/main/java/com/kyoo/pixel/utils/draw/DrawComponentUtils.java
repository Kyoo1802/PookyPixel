...
import com.kyoo.pixel.views.stage.fixture.components.LedComponent;
...
public final class DrawComponentUtils {
  ...
  public static void getBackground(
      GraphicsContext gc, Dimension size, FixtureProperties properties) {
    // Draw cached Image
  ...
  public static Image loadBackground(Dimension size, FixtureProperties properties) {
    return DrawUtils.OBJECTS_CACHE.get(
        "Background-" + size,
        o -> {
          Dimension canvasDimension = PositionUtils.toCanvasDimension(size);
          BufferedImage bufferedImage =
              new BufferedImage(
                  canvasDimension.width, canvasDimension.height, BufferedImage.TYPE_INT_ARGB);
          ...
          for (int i = 0; i < MAX_HORIZONTAL_LEDS; i++) {
            for (int j = 0; j < MAX_VERTICAL_LEDS; j++) {
              ...
              g2d.fillOval(
                  mousePoint.x + PositionUtils.HALF_SQUARE_LENGTH - DOT_SIZE / 2,
                  mousePoint.y + PositionUtils.HALF_SQUARE_LENGTH - DOT_SIZE / 2,
                  DOT_SIZE,
                  DOT_SIZE);
            }
          }
  ...
  public static void drawLedComponent(
      GraphicsContext gc, FixtureProperties properties, LedComponent ledComponent) {
    Image img =
        componentCache.get(
            ledComponent,
            component -> {
              ...
              BufferedImage bufferedImage =
                  new BufferedImage(
                      ledComponentSize.width,
                      ledComponentSize.height,
  ...
  public static void drawBridge(GraphicsContext gc, FixtureProperties properties, Bridge bridge) {
  ...
  protected static Point getBridgePosition(Component c, boolean isStart) {
    if (c instanceof LedComponent) {
      ...
      return PositionUtils.toCanvasPosition(
          lc.getPosition().y + l.getPosition().y, lc.getPosition().x + l.getPosition().x);
    } else {
      return PositionUtils.toCanvasPosition(c.getPosition().y, c.getPosition().x);
    }
  }

  public static void drawComponentSelection(
      GraphicsContext gc, FixtureProperties properties, ComponentUnit componentUnit) {
  ...
  private static void drawResizePoints(
      GraphicsContext gc, String hexColor, Point startPosition, Point endPosition) {
    ...
    gc.fillRect(
        startPosition.x,
        endPosition.y + PositionUtils.HALF_SQUARE_LENGTH,
        PositionUtils.HALF_SQUARE_LENGTH,
        PositionUtils.HALF_SQUARE_LENGTH);

    gc.fillRect(
        endPosition.x + PositionUtils.HALF_SQUARE_LENGTH,
        startPosition.y,
        PositionUtils.HALF_SQUARE_LENGTH,
        PositionUtils.HALF_SQUARE_LENGTH);
