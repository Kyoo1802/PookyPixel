...
import com.kyoo.pixel.fixtures.components.ComponentManager;
...
public final class InteractiveComponentManager {
  ...
  public void clearSelection() {
    ...
    selectedComponents()
        .forEach(iComponent -> iComponent.unSelect());
  }

  public void moveSelectedComponents(Point movement) {
    selectedComponents()
        .forEach(iComponent -> iComponent.move(movement));
  }
  ...
  public List<InteractiveComponent> getInteractiveComponent(List<ComponentKey> keysToMove) {
    return keysToMove.parallelStream().map(k -> getInteractiveComponent(k))
