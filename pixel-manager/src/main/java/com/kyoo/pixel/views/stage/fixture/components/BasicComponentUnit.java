...
public class BasicComponentUnit implements ComponentUnit {
  ...
  @Getter protected ComponentKey key;
  @Getter protected Map<String, Object> properties;
  @Setter @Getter protected Point position;
  @Setter @Getter protected Dimension size;
  ...
  public boolean intersects(Point point) {
    return position.x <= point.x
        && point.x < position.x + size.width - 1
        && position.y <= point.y
        && point.y < position.y + size.height - 1;
  }
}
